...
describe('HistoryManager in disconnected mode', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(LocalDB.prototype, 'addHistory').mockImplementation(mockDB.addHistory);
    vi.spyOn(LocalDB.prototype, 'getUnsyncedHistory').mockImplementation(mockDB.getUnsyncedHistory);
    vi.spyOn(LocalDB.prototype, 'markAsSynced').mockImplementation(mockDB.markAsSynced);
    vi.spyOn(LocalDB.prototype, 'getAllHistory').mockImplementation(mockDB.getAllHistory);
    vi.spyOn(LocalDB.prototype, 'setSyncMeta').mockImplementation(mockDB.setSyncMeta);
    vi.spyOn(LocalDB.prototype, 'getSyncMeta').mockImplementation(mockDB.getSyncMeta);
  ...
  it('should initialize without creating sync group when offline', async () => {
    ...
    expect(mockDB.setSyncMeta).toHaveBeenCalledWith('deviceId', expect.any(String));
  ...
  it('should store history locally when offline', async () => {
    ...
    const [listenerCallback] = mockChrome.history.onVisited.addListener.mock.calls[0];
  ...
  it('should queue items for sync when offline', async () => {
    isOnline = false;
    const historyItems = [
      { url: 'https://example1.com', title: 'Example 1', lastVisitTime: Date.now() },
      { url: 'https://example2.com', title: 'Example 2', lastVisitTime: Date.now() }
    ];
  ...
  it('should sync queued items when coming back online', async () => {
    ...
    isOnline = false;
    const historyItems = [
      { url: 'https://example1.com', title: 'Example 1', lastVisitTime: Date.now(), id: 1 },
      { url: 'https://example2.com', title: 'Example 2', lastVisitTime: Date.now(), id: 2 }
    ];
  ...
  it('should merge remote history when coming back online', async () => {
    ...
    isOnline = true;
    const remoteUpdates = [
      { url: 'https://example1.com', title: 'encrypted:Example 1 Updated', lastVisitTime: 3000 },
      { url: 'https://example3.com', title: 'encrypted:Example 3', lastVisitTime: 4000 }
    ];
